...
| Scorers: `random`, `length`, `dsir`, `perplexity`, `embed_similarity` | done, all verified on GPU |
...
| Select | `embed_similarity` on 300 PubMed docs, target MedMCQA | pool mean cosine 0.748 against selected 0.827; identical under `--stream` at `fit-size 16` | seconds |
