...
Stages are the exception. `STAGES` in `src/medsel/stages/__init__.py` is a plain dict, because
the three-stage pipeline is fixed by the proposal and is not meant to grow.
